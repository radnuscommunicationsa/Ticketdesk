...
function requireAdmin() {
    requireLogin();
    if (!isAdmin()) redirect(SITE_URL . '/employee/dashboard.php');
}
...
function getPriorityBadge($priority) {
    ...
    $p = $map[$priority] ?? [ucfirst($priority), 'low'];
    return '<span class="priority '.$p[1].'">'.$p[0].'</span>';
}
...
function getStatusBadge($status) {
    ...
    $s = $map[$status] ?? [ucfirst($status), 'open'];
    return '<span class="status '.$s[1].'">'.$s[0].'</span>';
}
